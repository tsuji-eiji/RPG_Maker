...
(() => {
  ...
  class PluginParam {
    ...
    _convert(param, paramName) {
      ...
      if (
        !PluginManagerEx.findTextParamSuffixList().some((regExp) =>
    ...
    _isStructArray(param) {
      return (
        Array.isArray(param) && (param.length === 0 || this._isStruct(param[0]))
      );
    }
  ...
  class PluginManagerEx {
    ...
    static createCommandArgs(args) {
      ...
      Object.keys(args).forEach((key) => {
        if (
          !this.findTextParamSuffixList().some((regExp) => key.match(regExp))
        ) {
